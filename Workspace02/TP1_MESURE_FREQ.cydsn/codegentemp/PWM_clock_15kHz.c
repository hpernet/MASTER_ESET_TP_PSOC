...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_Start
********************************************************************************
*
* Summary:
...
void PWM_clock_15kHz_Start(void) 
{
    ...
    PWM_clock_15kHz_CLKEN |= PWM_clock_15kHz_CLKEN_MASK;
	PWM_clock_15kHz_CLKSTBY |= PWM_clock_15kHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: PWM_clock_15kHz_Stop
********************************************************************************
*
* Summary:
...
void PWM_clock_15kHz_Stop(void) 
{
    ...
    PWM_clock_15kHz_CLKEN &= (uint8)(~PWM_clock_15kHz_CLKEN_MASK);
	PWM_clock_15kHz_CLKSTBY &= (uint8)(~PWM_clock_15kHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((PWM_clock_15kHz_CLKEN & PWM_clock_15kHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = PWM_clock_15kHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = PWM_clock_15kHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* PWM_clock_15kHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(PWM_clock_15kHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(PWM_clock_15kHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = PWM_clock_15kHz_GetSourceRegister();
    uint16 oldDivider = PWM_clock_15kHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = PWM_clock_15kHz_CLKEN & PWM_clock_15kHz_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(PWM_clock_15kHz_DIV_PTR, clkDivider);
                PWM_clock_15kHz_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                PWM_clock_15kHz_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(PWM_clock_15kHz_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((PWM_clock_15kHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(PWM_clock_15kHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: PWM_clock_15kHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_clock_15kHz_MOD_SRC |= modeBitMask & (uint8)PWM_clock_15kHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: PWM_clock_15kHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    PWM_clock_15kHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(PWM_clock_15kHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: PWM_clock_15kHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_clock_15kHz_MOD_SRC & (uint8)(PWM_clock_15kHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: PWM_clock_15kHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = PWM_clock_15kHz_GetDividerRegister();
    uint8 oldSrc = PWM_clock_15kHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        PWM_clock_15kHz_MOD_SRC |= CYCLK_SSS;
        PWM_clock_15kHz_MOD_SRC =
            (PWM_clock_15kHz_MOD_SRC & (uint8)(~PWM_clock_15kHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return PWM_clock_15kHz_MOD_SRC & PWM_clock_15kHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: PWM_clock_15kHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    PWM_clock_15kHz_PHASE = clkPhase & PWM_clock_15kHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: PWM_clock_15kHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return PWM_clock_15kHz_PHASE & PWM_clock_15kHz_PHASE_MASK;
}
